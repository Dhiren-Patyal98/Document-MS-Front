...
const AddIssuedEnventory = () => {
  ...
  const [getBranchName, setBranch] = useState([]);
  const [ItemName, setItems] = useState([]);
  useEffect(() => {
    getUsers();
    getInventory();
    getBranch();
  ...
  const handleBranchChange = async (selectedBranchId) => {
    axios
      .get(
        `${process.env.REACT_APP_API_URL}/items/getdata?query=${selectedBranchId}`
      )
      .then((response) => {
        const items = response.data;
        setItems(items);
            ...
            >
              <Row gutter={24}>
                <Col span={8}>
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: "Please input a valid Branch name!",
                        trigger: "change",
                      },
                    ]}
                  >
                ...
                <Col span={8}>
                  ...
                  >
                    <Input className="quantity" value={1} readOnly />
                  </Form.Item>
                </Col>
                <Col span={24}>
                  <Form.Item>
