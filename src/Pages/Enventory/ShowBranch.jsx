...
const ShowBranch = () => {
  ...
  const deleteItem = (itemId) => {
    axios
      .delete(`${process.env.REACT_APP_API_URL}/delete_branch/${itemId}`)
      .then((response) => {
  ...
  const handleSubmit = (values) => {
    axios
      .post(process.env.REACT_APP_API_URL + "/add-branch", values)
      .then((res) => {
        if (res != "") {
          getBranch();
          setIsModalOpen(false);
        ...
        <div style={{ padding: "30px" }}>
          <Row>
            <Col span={24} style={{ marginBottom: "30px" }}>
              <span className="popupTitle">Add New Branch</span>
            </Col>
