...
const ShowAllEmpolyees = () => {
  ...
  const handleSubmit = (values) => {
    ...
    data.append("ref_id", null);
    data.append("old_emp", false);

    axios
      .post(process.env.REACT_APP_API_URL + "/addNewEmployee", data)
      .then((res) => {
        setLoading(false);
        handleCancel();
        form.resetFields();
  ...
  return (
    <div>
                    ...
                    <Col span={24} style={{ marginBottom: "20px" }}>
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: "Please input Employee Image",
                          },
                        ]}
                      >
      ...
      >
        {allEmp.map((x, i) => {
          let { _id, f_name, l_name, img, designation, ref_id } = x;
          if (x.old_emp == "false") {
            return (
              <Col xs={24} sm={24} md={8} lg={6} key={i}>
                <EmployeeCard
                  id={_id}
                  ref_id={ref_id}
                  img={`${process.env.REACT_APP_API_URL}/images/${img}`}
                  name={`${f_name} ${l_name}`}
